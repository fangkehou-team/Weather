...
import org.eu.fangkehou.weather.citypicker.adapter.decoration.GridItemDecoration;
...
public class CityListAdapter extends RecyclerView.Adapter<CityListAdapter.BaseViewHolder> {
    ...
    private static final int VIEW_TYPE_CHOSEN = 11;

    private Context mContext;
    private List<City> mData;
    private List<ChosenCity> mChosenData;
    ...
    public CityListAdapter(Context context, List<City> data, List<ChosenCity> chosenData, int state) {
        this.mData = data;
        this.mContext = context;
        this.mChosenData = chosenData;
    ...
    public BaseViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        switch (viewType){
            ...
            case VIEW_TYPE_CHOSEN:
                view = LayoutInflater.from(mContext).inflate(R.layout.cp_list_item_choosen_layout, parent, false);
    ...
    public void onBindViewHolder(@NonNull BaseViewHolder holder, int position) {
        ...
        //热门城市
        if (holder instanceof ChosenViewHolder){
            final int pos = holder.getAdapterPosition();
            final City data = mData.get(pos);
            if (data == null) return;
            GridListAdapter mAdapter = new GridListAdapter(mContext, mChosenData);
            mAdapter.setInnerListener(mInnerListener);
            ((ChosenViewHolder) holder).mRecyclerView.setAdapter(mAdapter);
        }
    }
    ...
    public int getItemViewType(int position) {
        ...
        if (position == 1 && TextUtils.equals("已", mData.get(position).getSection().substring(0, 1)))
            return VIEW_TYPE_CHOSEN;
    ...
    public static class ChosenViewHolder extends BaseViewHolder {
        RecyclerView mRecyclerView;

        ChosenViewHolder(View itemView) {
            super(itemView);
